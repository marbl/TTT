...
import glob
bad_tangle_file = "/gpfs/gsfs11/users/antipovd2/res/TTT_paper/HG002/redefine_tangles/bad_tangles.list"
good_tangle_file = "/gpfs/gsfs11/users/antipovd2/res/TTT_paper/HG002/redefine_tangles/good_tangles.list"
prefix = "/gpfs/gsfs11/users/antipovd2/res/TTT_paper/HG002/redefine_tangles/"
...
def extract_scores_from_log(log_path):
    ...
    try:
        ...
        if mip_matches:
            mip_score = float(mip_matches[0])
            alt_score = float(mip_matches[-1])
            if alt_score < mip_score:
                mip_score = alt_score    
        ...
        not_found_match = re.search(not_found_pattern, content)
        if not_found_match:
            not_found_fraction = float(not_found_match.group(1))
...
def main():
    ...
    log_files = glob.glob(f"{prefix}/tangles/tangle_*/TTT_10/traversal.log")
    ...
    green_points = get_problematic_ids(good_tangle_file)
    # Create the plot
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(x_values, y_values, color='grey', alpha=0.7, s=15)

    # Highlight problematic points in orange and good points in blue
    for red_point in red_points:
        if red_point in labels:
            idx = labels.index(red_point)
            plt.scatter(x_values[idx], y_values[idx], color='orange', s=15)
    for green_point in green_points:
        if green_point in labels:
            idx = labels.index(green_point)
            plt.scatter(x_values[idx], y_values[idx], color='blue', s=15)
    ...
    plt.xlabel('Coverage inconsistency (X)')
    plt.ylabel('Not found alignments fraction (Y)')
    plt.title('Coverage vs alignments inconsistencies')
