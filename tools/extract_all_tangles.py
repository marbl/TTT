            ...
            if abs(verkko_res_path[start_id]) in abs_borders and abs(verkko_res_path[end_id]) in abs_borders:
                ...
                if sub_path[0] != borders[0] or sub_path[-1] != borders[1]:
                    ...
                    if len (alignment) != len(sub_path):
                        match = False
                        logging.info(f"Paths do not match in length {len(alignment)} vs {len(sub_path)}, borders {','.join([node_id_mapper.node_id_to_name_safe(b) for b in borders])}, verkko_path {','.join([node_id_mapper.node_id_to_name_safe(n) for n in verkko_res_path])}")                            
                    for i in range(min (len(alignment), len(sub_path))):
                        if alignment[i] != sub_path[i]:
                            match = False
                            logging.info (f"Paths do not match for borders {','.join([node_id_mapper.node_id_to_name_safe(b) for b in borders])}, verkko_path {','.join([node_id_mapper.node_id_to_name_safe(n) for n in verkko_res_path])}")
                            logging.info(f"First divergence at {node_id_mapper.node_id_to_name_safe(alignment[i])} vs {node_id_mapper.node_id_to_name_safe(sub_path[i])}, borders {','.join([node_id_mapper.node_id_to_name_safe(b) for b in borders])}, verkko_path {','.join([node_id_mapper.node_id_to_name_safe(n) for n in verkko_res_path])}")
                            break
                    if match:
